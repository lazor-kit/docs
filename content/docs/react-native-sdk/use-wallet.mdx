...
`Promise<BrowserResult>` - Object containing the signature and raw authenticator data.
...
| `~.feeToken` | `string` | No | SPL token mint for gas fees. |
| `~.computeUnitLimit` | `number` | No | Custom compute unit limit. |
| `~.addressLookupTableAccounts` | `AddressLookupTableAccount[]` | No | ALTs for versioned txs. |
| `~.clusterSimulation` | `'devnet' \| 'mainnet'` | Yes | Cluster to simulate against. |
| `options` | `object` | Yes | Options object. |
| `options.redirectUrl` | `string` | Yes | App deep link. |
